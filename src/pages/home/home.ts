...
trustedVideoUrl: Array<SafeResourceUrl> = [];
    ...
    notification = [];
    text;
    constructor(public navCtrl: NavController, public authData: AuthData, private domSanitizer: DomSanitizer,public popoverCtrl: PopoverController) {
    this.yuva = "workshop";
    this.addVideoUrls();
    this.showWorkshops();
    this.showEvents();
  ...
  //this.trustedVideoUrl.push(this.domSanitizer.bypassSecurityTrustResourceUrl(this.video)); 
    ...
    userref.update(l);
    firebase.database().ref('notification/').on('value',resp => {
    this.notification = snapshotToArray(resp);
    //this.text =  this.notification[0].text;
    });
  }
